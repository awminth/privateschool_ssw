...
if($action == 'show'){  
    ...
    $a="";
    if($pay!=""){
        $a=" and PayDescription='{$pay}'  ";
    }
   
    $search = $_POST['search'];
    if($search == ''){        
        $sql="select * from tblcms where Status=1 and Date>='{$from}' and Date<='{$to}' ".$a." 
        order by AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select * from tblcms where Status=1 
        and Date>='{$from}' and Date<='{$to}' ".$a." 
         and (PayDescription like '%$search%'
         or PayStatus like '%$search%') order by AID desc limit {$offset},{$limit_per_page}";        
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        <tr>
            ...
            <th>'.$lang['cms_paystatus'].'</th>
            <th>'.$lang['cms_amount'].'</th>
            <th>'.$lang['cms_paydescription'].'</th>
            <th>'.$lang['cms_date'].'</th>        
        </tr>
        </thead>
        <tbody>
        ...
        $sql_total="";
        if($search == ''){        
            $sql_total="select * from tblcms where Status=1 and Date>='{$from}' and Date<='{$to}' ".$a." 
              order by AID desc           
            ";
        }else{
            $sql_total="select * from tblcms where Status=1 and 
            and Date>='{$from}' and Date<='{$to}' ".$a."  
             and (PayDescription like '%$search%'
             or PayStatus like '%$search%') order by AID desc";        
        }
...
if($_POST["action"] == 'excel')
{
    ...
    $out="";
    $fileName = "ExpenseCMSReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                    <td colspan="5" align="center"><h3>Expense CMS</h3></td>
            </tr>
            <tr><td colspan="5"><td></tr>
            <tr><td colspan="5"><td></tr>
            <tr>  
                <th style="border: 1px solid ;">'.$lang['no'].'</th>  
                <th style="border: 1px solid ;">'.$lang['cms_paystatus'].'</th>  
                <th style="border: 1px solid ;">'.$lang['cms_amount'].'</th>  
                <th style="border: 1px solid ;">'.$lang['cms_paydescription'].'</th>  
                <th style="border: 1px solid ;">'.$lang['cms_date'].'</th> 
            </tr>';
        $no=0;
        while($row = mysqli_fetch_array($result))
        {
            ...
            $out .= '
                <tr>  
                    ...
                    <td style="border: 1px solid ;">'.$row["PayDescription"].'</td>  
                    <td style="border: 1px solid ;">'.$row["Amt"].'</td>
                    <td style="border: 1px solid ;">'.$row["PayStatus"].'</td>  
...
if($_POST["action"] == 'pdf')
{
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        $file = 'ExpenseCMSPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        echo "No Record Found.";
    }   
...
if($action == 'show1'){  
    ...
    $search = $_POST['search'];
    if($search == ''){        
        $sql="select * from tblcms where Status=0 
        and Date>='{$from}' and Date<='{$to}' ".$a." 
        order by AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select * from tblcms where Status=0 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){        
            $sql_total="select * from tblcms where Status=0 
            and Date>='{$from}' and Date<='{$to}' ".$a."   order by AID desc           
            ";
        }else{
            $sql_total="select * from tblcms where Status=0 
            and Date>='{$from}' and Date<='{$to}' ".$a." 
...
if($_POST["action"] == 'excel1')
{
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "IncomeCMSReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                    <td colspan="5" align="center"><h3>InCome CMS</h3></td>
            </tr>
            <tr><td colspan="5"><td></tr>
            <tr><td colspan="5"><td></tr>
            <tr>  
...
if($_POST["action"] == 'pdf1')
{
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        $mpdf->WriteHTML($stylesheet,1);  
        $mpdf->WriteHTML($out,2);
        $file = 'IncomeCMSPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        echo "No Record Found.";
    }   
