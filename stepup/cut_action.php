...
if($action == 'show'){  
    ...
    $search = $_POST['search'];
    if($search == ''){        
        $sql="select * from tblcut   where AID is not null  order by AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select * from tblcut    where AID is not null and Name like '%$search%' order by AID desc limit {$offset},{$limit_per_page}";        
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){
            $sql_total="select * from tblcut   where AID is not null";
        }else{
            $sql_total="select * from tblcut    where AID is not null and Name like '%$search%'";
        }
...
if($action == 'save'){
    ...
    $sql = "insert into tblcut (Name,LoginID) values ('{$category}',{$userid})";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Cut အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblcut    where AID=$aid";
...
if($action == 'update'){
    ...
    $sql = "update tblcut    set Name='{$category}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Cut အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblcut    where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Cut အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($_POST["action"] == 'excel')
{
    $search = $_POST['ser'];
    if($search == ''){        
        $sql="select * from tblcut where AID is not null";
    }else{
        $sql="select * from tblcut where AID is not null and Name like '%$search%'";        
    }

    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "CutReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                    <td colspan="2" align="center"><h3>Cut</h3></td>
            </tr>
