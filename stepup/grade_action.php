...
if($action == 'show'){  
    ...
    $sql="select * from tblgrade where AID is not null ".$a." 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="select * from tblgrade where AID is not null ".$a." 
        order by AID desc";
...
if($action == 'save'){
    ...
    $feeamt = $_POST["feeamt"];
    $sql = "insert into tblgrade  (Name,FeeAmount) values ('{$category}','{$feeamt}')";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Grade အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblgrade  where AID=$aid";
            ...
            $out.="<div class='modal-body'>
                ...
                <input type='hidden' id='action' name='action' value='editsave' />                              
                    ...
                    <div class='form-group' style='display:none;'>
                        ...
                        <input type='text' class='form-control border-success' id='feeamt1' name='feeamt1' value='{$row['FeeAmount']}'>
                    </div>                             
                </div>
...
if($action == 'update'){
    ...
    $category = $_POST["category"];
    $feeamt = $_POST["feeamt"];
    $sql = "update tblgrade  set Name='{$category}',FeeAmount='{$feeamt}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Grade အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblgrade  where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Grade အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "GradeReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        while($row = mysqli_fetch_array($result))
        {
            ...
            $out .= '
                <tr>  
                    ...
                    <td style="border: 1px solid ;">'.number_format($row["FeeAmount"]).'</td>             
                </tr>';
        }
        $out .= '</table>';
