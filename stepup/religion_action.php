...
if($action == 'show'){  
    ...
    $sql="select * from tblreligion where AID is not null ".$a." 
...
if($action == 'save'){
    ...
    $sql = "insert into tblreligion  (Name,LoginID) values ('{$category}',{$userid})";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Religion အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblreligion  where AID=$aid";
...
if($action == 'update'){
    ...
    $sql = "update tblreligion  set Name='{$category}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Religion အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblreligion  where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Religion အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($_POST["action"] == 'excel'){
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        <table >  
            <tr>
                    <td colspan="2" align="center"><h3>Religion</h3></td>
            </tr>
