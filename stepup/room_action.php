...
if($action == 'show'){  
    ...
    $sql="select r.*,g.Name as gname from tblroom r,tblgrade g  where r.GradeID=g.AID ".$a."  
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="select r.*,g.Name as gname from tblroom r,tblgrade g  where r.GradeID=g.AID ".$a."  
        order by AID desc";
...
if($action == 'save'){
    ...
    $grade = $_POST["grade"];
    $chk = "select AID from tblroom where Name='{$category}' and LoginID={$userid} and GradeID={$grade}";
    if(GetBool($chk)){
        echo 2;
    }else{
        $sql = "insert into tblroom   (Name,GradeID,LoginID) values ('{$category}',{$grade},{$userid})";
        if(mysqli_query($con,$sql)){
            save_log($_SESSION["username"]." သည် Room အားအသစ်သွင်းသွားသည်။");
            echo 1;
        }else{
            echo 0;
        }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select r.*,g.Name as gname from tblroom r,tblgrade g  where r.GradeID=g.AID and r.AID=$aid";
            ...
            $out.="<div class='modal-body'>
                ...
                <input type='hidden' id='action' name='action' value='editsave' />   
                    <div class='form-group'>
                        ...
                        <select class='form-control boder-success' name='gradeone'>
                            <option value='{$row['GradeID']}'>{$row['gname']}</option>
                            ".load_grade()."
                        </select>
                    </div>                           
                    <div class='form-group'>
...
if($action == 'update'){
    ...
    $category = $_POST["category"];
    $grade = $_POST["grade"];
    $chk = "select AID from tblroom where Name='{$category}' and AID!={$aid}";
    if(GetBool($chk)){
        echo 2;
    }else{
        $sql = "update tblroom   set Name='{$category}',GradeID='{$grade}' where AID=$aid";
        if(mysqli_query($con,$sql)){
            save_log($_SESSION["username"]."သည် Room အား update လုပ်သွားသည်။");
            echo 1;
        }
        else{
            echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblroom   where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Room အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($_POST["action"] == 'excel'){
    ...
    $sql="select * from tblroom  where AID is not null ".$a."  
    order by AID desc";
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "RoomReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                <td colspan="2" align="center"><h3>Room</h3></td>
            </tr>
