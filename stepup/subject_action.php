...
if($action == 'show'){  
    ...
    $search = $_POST['search'];
    if($search == ''){        
        $sql="select s.*,g.Name as gname from tblsubject s,tblgrade g
         where s.GradeID=g.AID order by s.AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select s.*,g.Name as gname from tblsubject s,tblgrade g
        where s.GradeID=g.AID and (s.Name like '%$search%' or g.Name like '%$search%') 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){        
            $sql_total="select s.AID from tblsubject s,tblgrade g
             where s.GradeID=g.AID order by s.AID desc";
        }else{
            $sql_total="select s.AID from tblsubject s,tblgrade g
            where s.GradeID=g.AID and (s.Name like '%$search%' or g.Name like '%$search%') 
            order by s.AID desc";        
...
if($action == 'save'){
    $name = $_POST["name"];
    $grade = $_POST["grade"];
    $sql = "insert into tblsubject (Name,GradeID,LoginID) values ('{$name}',{$grade},{$userid})";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Subject အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
            ...
            $out.="<div class='modal-body'>
                ...
                <input type='hidden' id='action' name='action' value='editsave' />    
                    ...
                    <div class='form-group'>
                        ...
                        <input type='text' class='form-control border-success' id='category1' name='name1' value='{$row['Name']}'>
                    </div>                               
                </div>
...
if($action == 'update'){
    ...
    $sql = "update tblsubject set Name='{$name}',GradeID={$grade} where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Subject အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblsubject where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Subject အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
