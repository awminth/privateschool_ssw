...
if($action == 'show'){  
    ...
    $search = $_POST['search'];
    if($search == ''){        
        $sql="select * from tblhostel   where AID is not null order by AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select * from tblhostel    where AID is not null and Name like '%$search%' order by AID desc limit {$offset},{$limit_per_page}";        
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){
            $sql_total="select * from tblhostel   where AID is not null";
        }else{
            $sql_total="select * from tblhostel    where AID is not null and Name like '%$search%'";
        }
...
if($action == 'save'){
    ...
    $sql = "insert into tblhostel    (Name,LoginID) values ('{$category}',{$userid})";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Hostel အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblhostel    where AID=$aid";
...
if($action == 'update'){
    ...
    $sql = "update tblhostel set Name='{$category}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Hostel အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblhostel    where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Hostel အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($_POST["action"] == 'excel')
{
    $search = $_POST['ser'];
    if($search == ''){        
        $sql="select * from tblhostel   where AID is not null";
    }else{
        $sql="select * from tblhostel    where AID is not null and Name like '%$search%'";        
    }
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        <table >  
            <tr>
                    <td colspan="2" align="center"><h3>Hostel</h3></td>
            </tr>
