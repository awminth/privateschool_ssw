...
if($action == 'showall'){ 
    $sql="select *,(select count(AID) from tblearstudent where 
     EARYearID=y.AID) as count 
    from tblearyear y where AID is not null  order by AID desc 
    ";
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        <tr>
            ...
            <th>'.$lang['h_academicyear'].'</th>
            <th>'.$lang['h_studentqty'].'</th>               
        </tr>
        </thead>
        <tbody>
        ';
...
if($action == 'show'){ 
    ...
    $sql="select *,(select count(AID) from tblearstudent where 
     EARYearID=y.AID) as count 
    from tblearyear y where AID is not null  ".$a." 
    order by AID desc 
...
if($action == 'save'){
    ...
    $sql = "insert into tblearyear (Name,LoginID) values ('{$category}',{$userid})";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် AcademicYear အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblearyear  where AID=$aid";
            ...
            $out.="<div class='modal-body'>
                ...
                <input type='hidden' id='action' name='action' value='editsave' />                              
                    <div class='form-group'>
                        <label for='usr'> ".$lang['ear_name'].":</label>
                        <input type='text' class='form-control border-success' id='category1' name='category1' value='{$row['Name']}'>
                    </div>                               
                </div>
                <div class='modal-footer'>
                    <button type='submit' id='btnupdate' class='btn btn-{$color}'><i class='fas fa-edit'></i>".$lang['btnedit']."</button>
                </div>";
        }
        echo $out;
    }
...
if($action == 'update'){
    ...
    $sql = "update tblearyear    set Name='{$category}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် AcademicYear အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblearyear    where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် AcademicYear အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "AcademicYearReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                <td colspan="3" align="center"><h3>'.$lang["ear_title"].'</h3></td>
            </tr>
            <tr><td colspan="3"><td></tr>
            <tr>  
                <th style="border: 1px solid ;">'.$lang["no"].'</th>  
                <th style="border: 1px solid ;">'.$lang["ear_academicyear"].'</th>  
                <th style="border: 1px solid ;">'.$lang["ear_stucount"].'</th>       
            </tr>';
        $no=0;
        while($row = mysqli_fetch_array($result))
        {
