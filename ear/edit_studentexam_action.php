...
$dt=date('Y-m-d');
$vno = $_SESSION["exam_earstudent_vno"];
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        <div class="text-right">
            <button id="btnsaveall" 
                ...
                <i class="fas fa-edit"></i>&nbsp;'.$lang["btnedit"].'
            </button>
        </div>
        ';
        echo $out;         
...
if($action=='save'){
    ...
    $sqlchk="select AID from tblexam where 
     EARStudentID={$stuid} and SubjectID={$subjectid} and VNO='{$vno}'";
    $res=mysqli_query($con,$sqlchk);    
    if(mysqli_num_rows($res) > 0){
        echo 2;
    }else{
        $sql="insert into tblexam (LoginID,EARStudentID,SubjectID,
        PayMark,GetMark,Result,D,Date,VNO,Chk) values ({$userid},'{$stuid}','{$subjectid}',
        '{$paymark}','{$getmark}','{$result}','{$d}','{$dt}','{$vno}',1)";      
...
if($action == "save_all"){
    ...
    $sql_del = "delete from tblexam_voucher where VNO='{$vno}'";
    if(mysqli_query($con,$sql_del)){
        $sql_v = "insert into tblexam_voucher (VNO,LoginID,EARStudentID,EARStudentName,
        EARYearID,EARYearName,GradeID,GradeName,ExamTypeID,TotalPayMark,TotalGetMark,Date) 
        values ('{$vno}','{$userid}','{$stuid}','{$stuname}','{$yearid}','{$yearname}',
        '{$gradeid}','{$gradename}','{$examtypeid}','{$totalpay}','{$totalget}','{$dt}')";
        if(mysqli_query($con,$sql_v)){
            save_log($_SESSION["username"]." သည် EAR Student exam အား edit သွားသည်။");
            echo 1;
        }else{
            echo 0;
        } 
