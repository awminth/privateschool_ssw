...
if($action == 'showteacher'){ 
    ...
    where LoginID={$userid} and Status=1 and 
...
if($action == 'editprepare'){ 
    ...
    $out.="
    <h3 class='text-center'>Staff Name : {$staffname}</h3>
    <br>        
    <table class='table table-bordered table-striped responsive' width='100%'>
        <thead>
        <tr>
            <td colspan='4'>{$lang['staff_basicsalary']}</td>               
        </tr>
        </thead>
        <tbody>
        <tr>
    ...
    $out.="</table>
        <div class='row'>
            <div class='col-2'><label>Remark</label></div>
            <div class='col-10'>
                <input type='text' name='rmk' class='form-control' />
            </div>
        </div>
...
if($action == 'pay'){
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Staff Salary Pay အားအသစ်သွင်းသွားသည်။");
        ...
        <div id='printdata'>
            ...
            {$lang['staff_salarymonth']} : ".enDate2($dt)."<br>
