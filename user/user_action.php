...
if($action == 'show'){  
    ...
    $search = $_POST['search'];
    if($search == ''){        
        $sql="select * from tbllogin where AID is not null  order by AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select * from tbllogin where AID is not null and UserName like '%$search%' order by AID desc limit {$offset},{$limit_per_page}";        
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){
            $sql_total="select * from tbllogin  where AID is not null";
        }else{
            $sql_total="select * from tbllogin  where AID is not null and UserName like '%$search%'";
        }
...
$sql = "insert into tbllogin (UserName,Password,UserType) values ('{$username}','{$password}',
0)";
...
$out.="<div class='modal-body'>
    ...
    <div class='form-group'>
        ...
        <input type='text' class='form-control border-success' name='username1' value='{$row['UserName']}'>
    </div>
    <div class='form-group'>
        <label for='usr'> Password :</label>
        <input type='password' class='form-control border-success' name='password1' value='{$row['Password']}'>
    </div>
</div>
...
$sql = "update tbllogin set UserName='{$username}',Password='{$password}' where AID=$aid";
if(mysqli_query($con,$sql)){
save_log($_SESSION["username"]."သည် User အား update လုပ်သွားသည်။");
echo 1;
}
else{
echo 0;
...
$aid = $_POST["aid"];
$sql = "delete from tbllogin where AID=$aid";
if(mysqli_query($con,$sql)){
...
$search = $_POST['ser'];
if($search == ''){
$sql="select * from tbllogin where AID is not null";
}else{
$sql="select * from tbllogin where AID is not null and UserName like '%$search%'";
}

$result = mysqli_query($con,$sql);
$out="";
$fileName = "UserReports-".date('d-m-Y').".xls";
if(mysqli_num_rows($result) > 0)
{
$out .= '
