...
if($action == 'show'){  
    ...
    $sql="select * from tblcategoryuniform where AID is not null ".$a." 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="select * from tblcategoryuniform where AID is not null ".$a." 
        order by AID desc";
...
if($action == 'save'){
    ...
    $sql = "insert into tblcategoryuniform  (Name) values ('{$category}')";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် uniform category အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblcategoryuniform  where AID=$aid";
...
if($action == 'update'){
    ...
    $sql = "update tblcategoryuniform  set Name='{$category}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Uniform Category အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblcategoryuniform  where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Uniform Category အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($_POST["action"] == 'excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "UniformCategory-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                    <td colspan="2" align="center"><h3>Uniform Category</h3></td>
            </tr>
...
if($_POST["action"] == 'pdf'){
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        $file = 'UniformCategoryPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        $out .= '
        <head><meta charset="UTF-8"></head>
