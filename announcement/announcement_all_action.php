...
if($action == 'save'){    
    ...
    $dt = $_POST["dt"];
    $sql = "insert into tblaccoumentall (Description,Date,LoginID)  
    values ('{$description}','{$dt}','{$userid}')";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် all announcement အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'edit'){   
    ...
    $dt = $_POST["edt"];
    $sql = "update tblaccoumentall set Description='{$description}',Date='{$dt}'  
    where AID={$aid}";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် All announcement အား update သွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'delete'){
    $aid = $_POST["aid"];
    $sql = "delete from tblaccoumentall where AID={$aid}";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် All announcement အားဖျက်သွားသည်။");
        echo 1;
    }else{
        echo 0;
    }    
...
if($_POST["action"] == 'excel'){
    ...
    $out="";
    $fileName = "AllAnnouncementReport_".date('d_m_Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '<head><meta charset="utf-8"></head>
        <table >  
            <tr>
                <td colspan="3" align="center"><h3>All Announcements</h3></td>
            </tr>
        ...
        <table >  
            ...
            <tr>
                <td style="border: 1px solid ;" colspan="3" align="center">No record found.</td>
            </tr>
        </table>';
        header('Content-Type: application/xls');
        header('Content-Disposition: attachment; filename='.$fileName);
...
if($_POST["action"] == 'pdf'){
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        $file = 'AllAnnouncementPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        $out .= '<head><meta charset="utf-8"></head>
        <table >  
