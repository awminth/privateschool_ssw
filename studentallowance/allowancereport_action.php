...
if($action == 'show'){  
    ...
    if($search == ''){        
        ...
        (select sum(Amount) from tblallowanceincome where StudentID=s.AID) as iamount,
        (select sum(Amount) from tblallowanceexpense where StudentID=s.AID) as eamount
        from tblstudentprofile s
        order by s.AID desc limit {$offset},{$limit_per_page}";
    }else{
        $sql="select s.Name as sname,
        (select sum(Amount) from tblallowanceincome where StudentID=s.AID) as iamount,
        (select sum(Amount) from tblallowanceexpense where StudentID=s.AID) as eamount
        from tblstudentprofile s where s.Name like '%$search%' 
        order by s.AID desc limit {$offset},{$limit_per_page}";        
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        if($search == ''){        
            ...
            (select sum(Amount) from tblallowanceincome where StudentID=s.AID) as iamount,
            (select sum(Amount) from tblallowanceexpense where StudentID=s.AID) as eamount
            from tblstudentprofile s order by s.AID desc";
        }else{
            $sql_total="select s.Name as sname,
            (select sum(Amount) from tblallowanceincome where StudentID=s.AID) as iamount,
            (select sum(Amount) from tblallowanceexpense where StudentID=s.AID) as eamount
