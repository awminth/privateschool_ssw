        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <table>
                        <tr>
                            <td><a href="<?=roothtml.'teacher/teachersalary.php'?>" class="btn btn-sm btn-<?=$color?>">
                                    <i class="fas fa-arrow-left text-white"></i>Back</a>
                            </td>
                            <td>
                                <h1>Teacher Salary Pay</h1>
                            </td>
                        </tr>
                    </table>
                </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card card-primary card-outline p-3">
                ...
                <table>
                    <tr>
                        <td><label for="inputEmail3" class="col-form-label"><?=$lang['tea_date']?></label></td>
                        <td><input type="month" class="form-control" name="dtmonth" value="<?=date('Y-m')?>"></td>
                        <td><button class="btn btn-<?=$color?>" id="btnfind"><?=$lang['tea_search']?></button></td>
                    </tr>
                </table>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    function load_pag(dtmonth) {
        $.ajax({
            type: "post",
            url: "<?php echo roothtml.'teacher/salarypay_action.php' ?>",
            data: {
                action: 'showteacher',
                dtmonth: dtmonth
            },
