...
if($action == 'show'){  
    ...
    where s.Status=0 and  Month(b.Date)='{$month}' 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        <tr>
            ...
            <th>'.$lang["tea_teachername"].'</th>
            <th>'.$lang["tea_description"].'</th>
            <th>'.$lang["tea_status"].'</th>
            <th>'.$lang["tea_amount"].'</th>
            <th>'.$lang["tea_date"].'</th>   
            <th>Action</th>     
        </tr>
        </thead>
...
if($action == 'excel'){
    ...
    $out="";
    $fileName = "TeacherBonus/CutReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0){
        $out .= '
        ...
        <table >  
            ...
            <tr>  
                ...
                <th style="border: 1px solid ;">'.$lang["tea_teachername"].'</th>  
                <th style="border: 1px solid ;">'.$lang["tea_description"].'</th>  
                <th style="border: 1px solid ;">'.$lang["tea_status"].'</th>  
                <th style="border: 1px solid ;">'.$lang["tea_amount"].'</th> 
                <th style="border: 1px solid ;">'.$lang["tea_date"].'</th> 
            </tr>';
        $no=0;
        while($row = mysqli_fetch_array($result)){
            $no = $no + 1;
...
if($action == 'pdf'){
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $file = 'TeacherBonus/CutPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        $out .= '
        <head><meta charset="UTF-8"></head>
...
if($action == 'save'){
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် teacher Bonus အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action=='editprepare'){
    $aid = $_POST["aid"];
    $sql = "select b.*,bo.Name as boname,c.Name as cname,s.Name as sname,Date(b.Date) as bdate from tblbcsalary b 
    inner join tblstaff s on b.StaffID=s.AID 
    left join tblbonus bo on bo.AID=b.BCID 
    left join tblcut c on c.AID=b.BCID 
     where  b.AID=$aid";
            ...
            $out.="<div class='modal-body'>
                ...
                <input type='hidden' id='action' name='action' value='editsave' />                              
                    <div class='form-group'>
                        <label for='usr'> {$lang["tea_teachername"]} :</label>
                    ...
                    <div class='form-group'>
                        <label for='usr'> {$lang["tea_status"]} :</label>
                    ...
                    <div class='form-group'>
                        <label for='usr'> {$lang["tea_amount"]} :</label>
                        <input type='number' class='form-control border-success' name='amt1' value='{$row['Amt']}' />
                    </div> 
                    <div class='form-group'>
                        <label for='usr'> {$lang["tea_date"]} :</label>
                        <input type='date' class='form-control border-success' name='date1' value='{$row['bdate']}' />
                    </div>                              
                </div>
                <div class='modal-footer'>
                    <button type='submit' id='btnupdate' class='btn btn-{$color}'><i class='fas fa-edit'></i>  {$lang["tea_save"]}</button>
                </div>";
        }
        echo $out;
    }
...
if($action == 'update'){
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် Teacher Salary Bonus/Cut အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Teacher Salary Bonus/Cut အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'show1'){
    ...
    where s.AID=p.StaffID and  s.Status=0  
...
if($action == 'delete1'){
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် Teacher Salary Pay အားဖျက်သွားသည်။");
        deletecms($vno);
        echo 1;
    }
    else{
...
if($action == 'excel1'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "SalaryPayReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0){
        $out .= '
        <head><meta charset="UTF-8"></head>
        <table>
...
if($action == 'pdf1'){
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $mpdf->WriteHTML($stylesheet,1);  
        $mpdf->WriteHTML($out,2);
        $file = 'SalaryPayPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        $out .= '
        <head><meta charset="UTF-8"></head>
...
if($action=='slipsalary'){   
    ...
    $out.="
        <div id='printdata'>
            ...
            {$lang["tea_salarymonth"]} : ".enDate2($date)."<br>
            ...
            <table class='table table\-bordered text\-sm' frame=hsides rules=rows width='100%'>
                ...
                <tr>
                    <td colspan='2' class='text-center'><b>Remark</b></td>  
